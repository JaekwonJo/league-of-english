...
### 오늘의 Top 3 (2025-11-22)
1. **기출문제 혁신**: `gpt-4o-mini` 기반 AI 파서를 도입해, 뒤죽박죽 섞인 PDF 텍스트도 문맥에 맞춰 완벽하게 구조화합니다. 텍스트 복사 붙여넣기로도 업로드 가능! 📝
2. **문제 품질 완벽화**: 어휘(반의어 강제), 빈칸(해설 번호 자동 교정), 요약(정답 논리 보장) 등 AI의 헛점을 원천 차단하는 로직을 완성했습니다. 🧠
3. **디자인 리뉴얼**: 홈 화면에 3D 크리스마스 트리와 유리 질감(Glassmorphism) UI를 적용해 연말 분위기를 물씬 풍깁니다. 🎄
...
- **어휘 문제 Fix**: AI가 원문 단어를 그대로 정답으로 내는 것을 방지하고, 반드시 문맥상 반의어를 사용하도록 강제했습니다.
- **빈칸 문제 Fix**: 보기 순서를 섞을 때 해설지의 번호(예: ③)가 꼬이는 문제를 해결했습니다. (서버가 해설 내용도 같이 고쳐줍니다)
- **UI/UX**: 홈 화면 3x3 그리드 메뉴, 3D 트리 애니메이션, 학습 헤더 상단 배치, 어법 문제 줄바꿈 개선 등 디테일을 잡았습니다.
- **서버 안정성**: CORS 화이트리스트 추가로 연결 끊김 방지, 아이디 찾기 API 추가.
