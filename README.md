...
> Latest update (2025-10-06): 빈칸 캐시에 축약 지문이 남지 않도록 원문 길이·문장 수를 저장하고 검증하는 가드가 추가됐어요. 리뷰 대기열·문제 라이브러리·PDF 내보내기도 함께 다듬어져 한 번 생성된 문제를 믿고 재활용할 수 있습니다. `npm test`와 `npm run lint`는 현재 그린 상태예요.

### 오늘의 Top 3
- `_normalizeBlankPayload`/`_acceptCachedProblem` 자동 테스트 추가 (이유: 길이·문장 검증이 다시 풀리지 않도록 회귀 테스트가 필요해요.)
- 복습 대기열 UX 다듬기 (이유: 학생이 틀린 문제를 모바일에서도 쉽게 찾고 재도전할 수 있어야 합니다.)
- 문제 라이브러리 검색/메모/히스토리 설계 (이유: 저장된 문제를 유형·난도로 관리하고 PDF 내보내기 기록을 남겨야 운영이 편합니다.)
