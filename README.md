...
> Latest update (2025-10-15): Study 설정·풀이·결과 화면을 `features/study` 모듈로 이사하고, 단계별 훅/컴포넌트로 나눠 관리·테스트 준비를 끝냈어요.

### 오늘의 Top 3
- 새 `useStudyConfig` 구조에 저장 세션 불러오기/자동 재시작 흐름을 연결하고, 로컬스토리지와 API 간 동기화를 점검해요. (세션 손실 방지)
- `ProblemDisplay` 공통 옵션 뷰에 👍/🚨 버튼과 신고 API를 묶어 문제 품질 피드백 루프를 완성해요. (콘텐츠 품질 관리)
- `StudyConfig`·`ProblemDisplay`·`ResultCard` 대상으로 렌더/상호작용 테스트를 추가해 리팩토링 회귀를 막아요. (안정성 확보)
