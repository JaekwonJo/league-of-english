...
> Latest update (2025-10-16): 어법 전용 `createGrammarPipeline`으로 생성 과정을 모듈화하고, 어휘 문제도 ①~④ 유형(1·2·3개 틀린 것 + 옳은 것 찾기)이 25%씩 섞이도록 확장했어요.

### 오늘의 Top 3
- 월고 기준 세트와 새로 생성한 문제를 자동으로 비교하는 회귀 파이프라인 만들기 → 스크립트는 준비됐으니 CI/운영 자동화까지 붙여 QA가 결과만 보면 돼요.
- 학생 학습 화면에 “실제 모의고사 미리보기” 모드 붙이기 → QA가 곧바로 눈으로 보고 오류를 잡을 수 있게 해요.
- 월고 2023·2022 PDF를 JSON으로 변환해 문제은행에 추가하기 → 같은 유형을 자주 보지 않도록 데이터 폭을 넓혀요.
