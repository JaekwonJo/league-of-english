...
### 오늘의 Top 3 (2025-11-12)
1. **원문 보존형 출제** – 빈칸은 `targetSpan`, 어휘는 `spans`로 위치만 지정하고 서버가 본문을 재구성합니다(원문 불변).
2. **분석 제목 EN+KR** – 영어 제목 옆에 한국어 해석을 함께 표기, 물음표(❓)는 목록 전체에서 1회만 표시합니다.
3. **네비/단어장 UX** – 상단 브랜드(크레스트+🦅) 클릭 시 홈 이동 + 텍스트 애니메이션, 단어장 개수 배지 제거 + 하단 독수리 포인터로 선택 유도.
