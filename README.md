...
> Latest update (2025-10-20): 어휘 문제는 밑줄 단어 의미를 고르는 5지선다로 재편되어 품사/예문 메타와 1~4단어 보기 검증까지 갖췄어요. 관리자 신고 보드는 상태·유형·문서 필터와 CSV 내보내기, 토스트 안내를 지원해 triage 속도가 빨라졌고, 로딩/결과 화면의 진행 막대와 랭킹 텍스트 대비를 다듬어 다크 모드에서도 또렷하게 보입니다.

### 오늘의 Top 3
- 어휘 생성 파이프라인을 대상으로 end-to-end 시나리오와 failure 캐시 클리너 테스트를 추가해 새 메뉴얼이 장기적으로도 안전하게 유지되도록 합니다. (출제 안정성)
- 신고 대시보드에 다중 선택 · 상태 일괄 변경 및 분석용 그래프(주간 신고 추이, 문서별 분포)를 붙여 QA 루틴을 더 빠르게 만듭니다. (운영 효율)
- 진행률/토스트 시스템을 공통 컴포넌트로 추출해 단어 시험·분석본 화면에도 재사용할 수 있게 하고, 사용자 가이드를 한글/이모지 버전으로 업데이트합니다. (UX 일관성)
...
| 학습 경험 | 타이머, 자동 채점, LP/랭크, 양방향 어휘·랜덤/순서 출제 | 단어 시험, 숙제 스케줄러, PWA | 스터디룸, 친구 비교, AI 튜터 |
