...
- dev:auto 추가: 빈 포트 자동 선택(백 5000+/프론트 3000+) 및 `REACT_APP_API_URL=http://localhost:{port}/api` 주입 → 포트 충돌·404 감소
- API 타임아웃 상향: 분석/세트/보카 생성·제출 등 장시간 요청에 공통 타임아웃 도우미 적용
- 보카 "채점 중" 무한대기 해결: 제출 전 시간 스냅샷·타이머 정리, 서버 요약 없으면 클라이언트가 요약 계산 폴백
- 결과 즉시 반영: 보카 제출 응답에 `summary/stats/rank/updatedUser` 포함 → LP/티어 UI 즉시 갱신
- 본문 품질: ①~⑤ 표식 자동 삽입, 밑줄 두께/색상 개선(가독성 향상), 출처 라벨 표준화(예: 고2 2025년 10월 모의고사)
- GitHub 푸시 안내: 비밀번호 대신 PAT/gh 사용법을 문서화(프롬프트 멈춤 오해 방지)
