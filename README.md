...
### 오늘의 Top 3 (베타 직전)
1. **모의고사 실전 모드 E2E 캡처** – 업로드→응시→해설 흐름을 Playwright로 자동화해 PDF 파서·타이머 회귀를 감시
2. **Render/Vercel 자동 배포 시크릿 세팅** – GitHub PAT와 Render/Vercel 시크릿을 정리해 main 푸시 즉시 배포
3. **랭킹 API 500 디버깅** – leaderboard/tier-distribution 데이터를 보강해 대시보드 신뢰도 확보
...
## 6. 최근 업데이트 (2025-11-06)
- 모의고사 실전 모드(50분 타이머, 채점/복습, 프로 전용 AI 해설)를 추가해 학생이 PDF 기반 시험을 앱에서 바로 응시할 수 있습니다.
- `mockExamService`가 PDF → 문제/정답 파싱과 캐시 리셋을 처리해 잘못된 업로드로 인한 서버 오류를 예방합니다.
- 관리자 페이지에 모의고사 문제지·정답 PDF 동시 업로드 카드를 도입해 코드 수정 없이 실전 데이터를 갱신할 수 있습니다.
- AnalysisPage와 VocabularyPage 상단 히어로를 고대비 그라데이션과 메트릭 카드로 재구성해 첫 화면에서 핵심 지표와 안내가 선명하게 보입니다.
- 분석 자료·관리자 리스트 UI를 고대비 카드/그라데이션 톤으로 통일해 데스크톱·모바일 모두 동일한 경험을 제공합니다.
- AnalysisPage 문서 목록·지문 카드가 고급스러운 글로우·호버 효과를 갖춰 탐색성이 개선되었습니다.
