...
> Latest update (2025-10-02): 밑줄 자동 복구까지는 안정화됐고, 이제 스터디 로딩 스피너 한글화·어법 세트 QA·클라이언트 자동화가 오늘의 핵심입니다 (`npm test`, `npm run lint` 여전히 통과). 함축적 의미 추론도 결정형 매뉴얼 + OpenAI 생성기로 맞춰, 다른 유형과 똑같이 API에서 뽑을 수 있게 정리했어요.
