...
> Latest update (2025-10-07): 상태 문서를 다시 맞춰 빈칸 가드 이후의 다음 행동(테스트, 모니터링, 재출제 제어)에 집중하게 정리했어요. 코드는 건드리지 않았고, 테스트는 이번 작업에서 새로 돌리지 않았습니다.

### 오늘의 Top 3
- `_normalizeBlankPayload`/`_acceptCachedProblem` + `listReviewQueueForUser`/`startManualSession` 테스트 작성 (이유: 방금 세운 빈칸·복습 가드가 다시 무너지지 않게 자동화 장치를 만들어요.)
- 빈칸 재생성 루프 모니터링·알림 도입 (이유: 지금은 서버 로그를 직접 봐야 해서 문제를 늦게 발견해요.)
- 재출제 확률을 설정/관리자 UI로 노출 (이유: 학생/반 상황에 따라 복습 빈도를 조정할 수 있게 만들어요.)

### 요금제 (초안)
- **무료**: 생성 즉시 DB에 저장하지 않는 체험용 문제만 제공되고, 응답 속도는 의도적으로 느려요.
- **프리미엄 (월 9,900원)**: 검증·저장된 고품질 문제를 바로 받고, 기본 학습 리포트를 볼 수 있어요.
- **프로 (월 19,900원)**: 프리미엄 기능 + 상세 분석본(유형별 통계, 추천 복습 코스)을 함께 제공해요.
