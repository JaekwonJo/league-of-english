...
> Latest update (2025-10-04): 관리자 문서를 학생에게 바로 공개할 수 있도록 `document_visibility` 규칙과 공유 모달(🌐)을 추가했어요. 학생 계정은 학교/학년/이메일 필터를 따라 문서를 보게 되고, 예전 DB 스키마 때문에 터지던 회원가입도 `password_hash` 백필로 막았습니다.

### 오늘의 Top 3
- Blank/Grammar 세트가 짧은 지문 때문에 실패하는 케이스를 잡아 재시도/대체 전략을 붙입니다. (학생 학습 중단 방지)
- 공유 규칙을 분석본/학습 화면에도 연결해 공개된 Variant만 학생이 열람하도록 만듭니다. (권한 일관성)
