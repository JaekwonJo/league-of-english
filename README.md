...
> Latest update (2025-10-02): 밑줄 자동 복구까지는 안정화됐고, 이제 스터디 로딩 스피너 한글화·어법 세트 QA·클라이언트 자동화가 오늘의 핵심입니다 (`npm test`, `npm run lint` 여전히 통과). 함축적 의미 추론과 흐름상 무관 문장 유형도 결정형 매뉴얼 + OpenAI 생성기로 맞춰, 다른 유형과 똑같이 API에서 뽑을 수 있게 정리했어요.

### 오늘의 Top 3
- 스터디 로딩 스피너/단어 예열 한글화 및 모지바케 제거 (이유: 첫 화면부터 한국어로 자연스럽게 보여야 학습 몰입이 깨지지 않아요.)
- `npm run dev:all`로 어법 세트 전체 흐름 QA (5개 밑줄 확인) (이유: OpenAI → API → React 흐름이 실전에서도 끊기지 않는지 한 번에 검증해요.)
- 클라이언트 린트/테스트 도입으로 스피너·카운트다운 회귀 방지 (이유: 앞으로 문자열·밑줄 버그가 다시 생기지 않도록 자동 안전망을 깔아요.)
