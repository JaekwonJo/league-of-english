...
### 오늘의 Top 3 (2025-11-17)
1. 빈칸 생성기 원문 엄수 – passage 미존재 시 즉시 오류를 띄우고, targetSpan을 역산해 정답/본문이 반드시 동일 구절이 되도록 강제했어요. ✅
2. 지문 필터링 – 150자 미만 본문 조각을 자동 제외 + 로그 기록으로 QA 재현이 쉬워졌어요. 🧹
3. 문제학습 현황 공유 – 2-23-11 QA 결과를 PROJECT_STATE/BUILDLOG/README에 동시에 반영해 누구나 상태를 한눈에 확인할 수 있어요. 📋

### 최근 업데이트
- 빈칸 생성기는 passage를 찾지 못하면 친절한 오류를 띄우고, normalize 단계에서 원문 prefix/suffix를 대조해 정답·본문이 1:1로 일치할 때만 저장됩니다.
...
- 사이드바 정리: "문제 보관함"을 맨 아래로 이동해 메뉴 흐름을 단순화했습니다.
 - 모바일 타이포: 어휘 세트 제목이 작은 화면에서 이쁘게 줄바꿈되도록 수정했습니다.
- 은은한 애니메이션: slowGradient를 36s로 통일하고, 분석 페이지는 카드 내부에서만 은은하게 반짝입니다.
- 워크북 네비: 데스크톱 하단 좌/우 고정 ‘이전/다음’ 버튼 가독성/음영을 강화했습니다.
- 해설 보기: 문제/모의고사 해설 박스 하단에 ‘↑ 맨 위로’ 버튼을 추가했습니다.
- 통계 구성: ‘단어 훈련 → 유형별 정답률 → 워크북 통계 → 모의고사’ 순으로 재배치하고 워크북 통계를 신설했습니다.
 - 로그인 상향 알림: premium/pro + 기간 표기 오버레이(세션당 1회).
 - 유지보수 배너: REACT_APP_MAINTENANCE=1일 때 상단 안내띠 표시.
 - B4 임포트: scripts/import-grammar-b4.js로 복습자료 PDF→Step 11 카드 자동화.
 - 선택 지문 강제 적용: 캐시/생성 모두 선택 지문 본문과 일치하는 항목만 사용하도록 강화했습니다.
 - AI 시간 예산: 기본 30초로 상향(환경변수 미설정 시)해 간헐적 타임아웃을 완화했습니다.
...
- 일부 오래된 문서는 passage JSON이 누락되어 있어 빈칸 생성 전에 다시 파싱하거나 길이를 보완해야 합니다.
