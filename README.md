...
> Latest update (2025-10-08): 어법·어휘 문제를 전용 메뉴얼/프롬프트로 분리해 학습 UI에서도 각각 선택할 수 있고, tier/app 설정의 색상도 CSS 변수로 통일했어요. 함축 추론 재시도는 `server/tmp/implicit-retries.log`에 variantTag와 실패 사유가 축적됩니다.

### 오늘의 Top 3
- StudyResult·Admin·Analysis 주요 컴포넌트에 테마 스냅샷 테스트를 붙여 CSS 변수 회귀를 빠르게 잡아요.
- `server/tmp/implicit-retries.log`를 관리자에서 바로 볼 수 있는 대시보드/알림으로 연결해요.
- ProblemLibrary 타입 토글 때 발생하는 플리커를 없애고 로딩 상태를 명확히 보여줘요.
