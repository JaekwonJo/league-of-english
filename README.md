...
> Latest update (2025-10-09): Render에서 이메일 인증 메일이 실제로 발송되지 않아 가입이 막혀 있어요. `EMAIL_FROM`을 `League of English <jaekwonim@naver.com>` 형식으로 맞추고, SMTP 오류 로그를 남기도록 정리했어요. 베타 배포 전에 환경/헬스/SMTP를 한 번에 확인하는 스모크 스크립트도 준비 중입니다.

### 오늘의 Top 3
- 네이버 SMTP에서 이메일 인증/알림이 실제로 발송되도록 `EMAIL_FROM` 형식을 손보고, 실패 로그를 남겨 원인을 정확히 찾습니다. (가입 막힘 해결)
- Render 배포 전에 환경 변수·헬스체크·SMTP 연결을 한 번에 검증하는 스모크 스크립트를 만듭니다. (베타 배포 안정성)
- PDF 단어장을 읽어 30문항 시험을 생성하는 단어 시험 파이프라인을 완성합니다. (콘텐츠 라인업 확장)
