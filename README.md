...
> Latest update (2025-10-15): Study 설정·풀이·결과 화면을 `features/study` 모듈로 이사하고, 문제 화면 👍/🚨 피드백과 학습 세션 이어하기까지 붙였어요.

### 오늘의 Top 3
- 문제 피드백이 쌓이는 `problem_feedback`을 관리자가 한 화면에서 검토·처리할 수 있는 대시보드를 만들어요. (품질 대응 속도)
- 좋아요/신고 API에 rate-limit와 기기 로그를 붙여 중복 클릭 스팸을 막아요. (신뢰도 향상)
- 저장된 학습 세션을 계정 단위로 동기화하고 복구 성공률을 로깅해 다중 디바이스 지원을 준비해요. (세션 안정성)
