...
### 오늘의 Top 3 (2025-11-15)
1. 모바일 Hero 레이아웃 고정 – 제목/부제 줄바꿈 정상화, 모바일에서는 오른쪽 CTA를 숨겨 텍스트 겹침 제거. 📱
2. 어휘 Day 가독성 – 2열 그리드 + 중립 카드(밝은 배경/진한 글자) + shimmer 제거로 선명하게. 🔍
3. 분석 모노톤 – 과한 그라데이션 제거, 카드/그리드는 모노톤 유지하고 ‘분석 보기’만 은은하게 강조. 🎯
4. 로그인 상향 알림 – 프리미엄/프로로 상향된 학생에게 기간 안내 오버레이 표시. 🎉
5. B4 어법 임포터 – B4 어법 복습 PDF의 본문+하단 `정답` 표를 읽어, 워크북 Step 11 카드(앞면 지문·뒷면 정답+해설 프롬프트)로 일괄 등록합니다. 🛠️
