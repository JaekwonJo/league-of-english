...
> Latest update (2025-10-18): 어법 생성 실패 로그를 읽어 밑줄 수정 지시문을 자동으로 붙이고, 학습 설정·복습·결과 화면을 테마 토큰·진행 막대·🔝 버튼으로 다듬어 다크 모드 가독성을 높였어요.

### 오늘의 Top 3
- 어법·빈칸을 포함한 AI 문제 6종 통합 테스트를 추가해 재시도 로직이 실제 OpenAI 호출에서도 "밑줄 변경" 검증을 통과하는지 자동 확인합니다. (출제 안정성)
- 문항 신고 대시보드에 유형/문서/사용자 필터와 CSV 내보내기, 429 토스트 안내를 붙여 품질 triage 속도를 높입니다. (운영 효율)
- 랭킹·세션 복구 플로우에 대한 E2E 시나리오를 작성해 다크 모드, 저장/재개, LP 반영을 베타 전에 반복 검증합니다. (베타 준비)
