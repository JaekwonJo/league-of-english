...
> Latest update (2025-10-10): WordMaster 등 단어장을 다시 파싱해 Day별 단어가 빠짐없이 저장되고, 어휘 시험이 티어 기반 타이머·이전/다음·양방향 문제로 업그레이드됐어요.

### 오늘의 Top 3
- 어법 보기 셔플과 정답 재매핑을 붙여 ③번 고정 정답 패턴을 없애요. (공정성 확보)
- 학습 세트 생성이 부분 실패했을 때 원인·진행률을 알려주는 안내 패널을 추가해요. (사용자 신뢰)
- Render 헬스 체크 라우트와 SMTP 스모크 테스트를 갖춰 배포 타임아웃을 없애요. (운영 안정성)
...
| 학습 경험 | 타이머, 자동 채점, LP/랭크, 양방향 어휘 시험 | 단어 시험, 숙제 스케줄러, PWA | 스터디룸, 친구 비교, AI 튜터 |
