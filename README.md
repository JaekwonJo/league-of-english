...
> Latest update (2025-10-13): 빈칸/어휘 파이프라인이 멈추는 원인과 학습 세트 전체 실패 증상을 다시 모아서, 바로 풀어야 할 우선순위를 정리했어요.

### 오늘의 Top 3
- 빈칸/어휘 생성기가 검증 단계에서 막히지 않도록 프롬프트·검증 로직을 손봐 학습 세트가 다시 돌아가게 만들어요. (핵심 기능 복구)
- 학습 세트가 일부 유형 실패로 중단돼도 이미 통과한 문제는 바로 제공하고, 실패한 유형은 재시도 큐로 보내요. (학생 경험 안정)
- 문제 화면에 "도움이 됐어요"·"신고" 버튼과 관리자 큐를 붙이고, 라이트/다크 대비도 함께 다듬어요. (품질 피드백 + 가독성)
