...
### 오늘의 Top 3 (베타 직전)
1. **모의고사/어휘 Playwright 시나리오** – 파서·히어로 리뉴얼 이후 업로드→응시→해설 흐름을 자동 캡처해 회귀를 감시
2. **지문 이름 편집 모달** – `prompt` 대신 inline 입력·검증·취소가 가능한 UI를 붙여 모바일에서도 커스텀 라벨을 다룰 수 있게 하기
3. **랭킹 API 시드/폴백 확장** – leaderboard/tier-distribution가 데이터 부족 시에도 500 대신 빈 상태 응답을 돌려주도록 서버 로직 보강
...
## 6. 최근 업데이트 (2025-11-07)
- 분석 자료 리스트/상세를 다크 톤 카드 UI로 재정비하고 지문 이름을 DB(`document_passage_labels`)에 저장·수정할 수 있게 했습니다.
- VocabularyPage 히어로를 안내 + CTA만 남기는 구조로 바꿔 "총 Day/누적 단어" 같은 혼란스러운 수치를 제거했습니다.
- 랭킹 페이지 상위 카드 대비와 실버 티어 아이콘(🥈)을 교체해 1~3위 이름과 LP가 어두운 배경에서도 또렷하게 보입니다.
- mockExamService가 PDF를 찾지 못하거나 `pdf-parse`가 실패하면 JSON 기본 데이터를 자동으로 사용하고, `모의고사원문/모의고사 원문` 폴더 모두를 확인하도록 보강했습니다.
- 분석 문장별 카드/어휘 메모의 배경·텍스트 색을 다크 테마에 맞춰 다시 지정해 모바일에서도 가독성을 확보했습니다.
