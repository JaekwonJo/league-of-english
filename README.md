...
> Latest update (2025-10-19): 학습 요청은 10문 제한으로 고정하고, 요약·빈칸 검증을 강화했어요. 어휘 문제는 (A)(B) 슬롯 대신 “본문과 가장 거리가 먼 것은?” 5지선다 구조로 바뀌었고, 생성 요약 패널은 `📦/🍞/✨` 이모지로 결과를 귀엽게 안내해요. 분석본 선택은 2개까지만 가능해져 품질 검토가 더 안정적이에요.

### 오늘의 Top 3
- 어휘 5지선다 생성이 안정적인지 전체 통합 테스트와 샘플 세트를 확장해, 모순 보기 검증이 CI에서 자동으로 잡히도록 합니다. (출제 안정성)
