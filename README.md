...
> Latest update (2025-10-21): 어법 문제 생성 메커니즘을 밑줄 추출→오류 태깅→해설 작성까지 다시 설계하고, fallback과 OpenAI 경로를 같은 검증기로 묶는 작업을 시작했어요. 최근 업로드 지문을 라벨링해 기준 데이터를 만들 준비를 하고 있습니다.

### 오늘의 Top 3
- 어법 문제 생성 메커니즘을 밑줄 추출 → 오류 태깅 → 해설/오답 근거까지 한 번에 다듬어, 짧은 지문에서도 보기 다섯 개가 자연스럽게 나오도록 다시 짜요. (콘텐츠 신뢰도)
- 문서 기반 fallback과 OpenAI 경로를 통합 테스트로 묶어 업로드 → problemSetService → 학습 화면까지 자동으로 검증해요. (배포 안전성)
- WordNet gloss를 한국어 뜻·반의어·예문으로 정리해 fallback 해설도 완전한 한국어로 제공되도록 사전을 만들어요. (학생 이해도)
