...
### 오늘의 Top 3 (2025-11-13)
1. **분석 화면 정리** – 영어 제목 옆 한국어 문구 제거로 중복(3회) 문제 해결. 이제 제목 2개, 작가의 주장, 한 줄 요약(영/한)만 표시돼요.
2. **빈칸 원문 보존 강화** – 서버가 "빈칸만 제외 동일"을 엄격히 검사해 일부 문장 누락/요약된 결과를 거부합니다.
3. **모델 선택 유연화** – `LOE_OPENAI_PRIMARY_MODEL/SECONDARY_MODEL/ANALYSIS_MODEL`로 gpt-4o ↔ 4.1/오더 계열 등 교체가 쉬워졌어요.
