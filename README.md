...
> Latest update (2025-10-14): CSAT 세트 서비스가 `problemSetService`로 분리되고, StudyPage가 StudyMode/ReviewMode 컴포넌트로 나뉘어 로딩·복습 UI를 재사용할 수 있게 정리했어요.

### 오늘의 Top 3
- `blank`/`vocabulary` 검증기를 완화해도 세트가 이어지도록 캐시 재활용·재시도 정책을 손봐요. (학습 중단 방지)
- 학습 화면에 👍/🚨 버튼과 라이트·다크 대비 조정을 붙여 문제 품질 신고와 가독성을 동시에 챙겨요. (사용성 + 품질 관리)
- 새 StudyMode/ReviewMode 구조에 저장 세션·테스트 커버리지를 연결해 프런트 리팩토링을 안전하게 마무리해요. (유지보수성 확보)
