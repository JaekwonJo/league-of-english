...
> Latest update (2025-10-17): 요약 문제 생성이 실패 사유를 분석해 스스로 교정하도록 개선했고, 다크 모드에서도 텍스트 대비가 깨지지 않도록 전역 색상 토큰을 정리했어요.

### 오늘의 Top 3
- 요약 문제 생성 실패를 재현하는 통합 테스트와 재시도 리포트를 추가해, `partial_generation` 로그 전에 원인을 바로 확인할 수 있게 합니다. (출제 안정성)
- 신고 데이터를 유형·문서·사용자 기준으로 필터링하고 CSV/일괄 처리를 붙여 관리자들이 빠르게 품질 결정을 내리도록 돕습니다. (품질 의사결정)
- 라이트/다크 테마를 자동 대비 테스트에 연결해 잔여 정적 CSS도 손쉽게 점검할 수 있게 합니다. (사용자 경험)
