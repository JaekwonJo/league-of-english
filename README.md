...
### 오늘의 Top 3 (베타 직전)
1. **랭킹 실데이터 시드** – 폴백 대신 실제 LP/티어/분포 데이터를 자동 집계해 홈 위젯을 실시간으로 채웁니다.
2. **라벨 변경 히스토리 뷰어** – 새 audit 로그를 관리자 UI에서 검색/롤백할 수 있는 도구를 붙입니다.
3. **Playwright 데이터 시나리오 확장** – E2E CI가 항상 통과하도록 샘플 문서/단어장을 자동 시드하고 캡처 아카이브를 정리합니다.
...
- 독수리 마스코트: 눈 깜빡임/날개 플랩/탭 점프 상호작용을 추가해 홈/어휘 화면에서 생동감을 높였습니다.
