...
> Latest update (2025-10-12): 어법 문제 보기가 본문 `<u>…</u>` 구간에서 자동으로 추출돼 ①~⑤ 옵션이 비거나 전체 문단이 밑줄로 보이는 현상을 막았습니다. OpenAI가 보기를 비워도 본문에서 추출한 텍스트로 최소 구성을 만들어 세트가 끊기지 않아요.

### 오늘의 Top 3
- Wolgo PDF에서 (A)~(E) 밑줄과 ①~⑤ 보기를 정확히 분리해 JSON으로 만드는 어법/어휘 구조화 파서를 완성합니다. (콘텐츠 신뢰도)
- 새 파서 결과를 `fallbackProblemFactory`·`aiProblemService` 검증 단계에 연결해, 수동 템플릿이 아닌 실제 기출 데이터를 우선 노출합니다. (생성 안정성)
- 어휘 JSON 스키마를 슬롯형/단일 밑줄형으로 분리하고, 파서가 반환한 메타를 두 유형에 맞춰 저장·노출하도록 정리합니다. (유형 일관성)
