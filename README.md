...
> Latest update (2025-10-01): 외부 어법·어휘 메뉴얼 경로를 그대로 읽어 정확한 프롬프트를 쓰고, tier/app JSON 색상을 CSS 변수로 맞춰 라이트·다크 테마가 똑같이 보여요. 함축 추론 재시도 로그는 `server/tmp/implicit-retries.log`에 JSONL로 쌓여요.

### 오늘의 Top 3
- Analysis 페이지를 DB에 저장된 요약·핵심 포인트·질문 데이터로 연결해 “분석 자료”가 바로 뜨게 만들어요.
- Vocabulary 학습 모드를 단어장 저장/반복/스피드모드로 확장해 단어 공부 루틴을 완성해요.
- 1주 내 베타 배포를 위해 Render/Vercel 기준 `.env`, 빌드 스크립트, 백업 절차를 정리해둬요.
