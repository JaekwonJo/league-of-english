...
router.get('/users/profile', verifyToken, async (req, res) => {
  try {
    const user = await database.get('SELECT id, username, email, name, school, grade, role, membership, points, last_login_at, created_at, updated_at FROM users WHERE id = ?', [req.user.id]);
...
router.put('/users/profile', verifyToken, async (req, res) => {
  try {
    ...
    if (!school && !name && gradeRaw === undefined) {
    ...
    const updated = await database.get('SELECT id, username, email, name, school, grade, role, membership, points, last_login_at, created_at, updated_at FROM users WHERE id = ?', [req.user.id]);
