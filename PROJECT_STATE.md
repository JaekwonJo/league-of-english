...
## Today’s Top 3 (2025-10-20)
1. **숨김 문항 회귀 테스트.** `setProblemActive` 흐름과 `/admin/problems/:id/(de)activate` 엔드포인트를 아우르는 통합·E2E 테스트를 마련해 숨긴 문제가 다시 노출되지 않는지 자동으로 검증합니다. (품질 안전망)
2. **숨긴 문제 인벤토리 UI.** 관리자 페이지에 “숨긴 문항” 탭과 복구 버튼을 추가해 신고 처리 후에도 누락 없이 상태를 관리하고, 숨김 사유/담당자 로그를 한눈에 정리합니다. (운영 효율)
3. **피드백 안내 강화.** 신고/좋아요 영역에 rate-limit 토스트·사유 프리셋·간단한 가이드 모달을 연결해 학생이 어떤 상황에서 🚨을 눌러야 하는지 바로 이해하도록 돕습니다. (UX 친절함)
...
- 숨긴 문항을 한눈에 모으는 인벤토리/복구 UI가 없어, 숨김 처리 후 어떤 문제가 제외됐는지 추적하기 어렵습니다.
...
- 요약 생성 검증에 (A)/(B) 주변 어휘 중복 검사와 소문자 1~4단어 제한을 추가해 "Establishing an opening routine" 같은 어색한 문장을 걸러냅니다.
- 빈칸 옵션은 명사구만 통과하도록 재검증하고, 매뉴얼 샘플도 noun phrase 위주로 교체해 "launching~" 형태가 나오면 재시도합니다.
