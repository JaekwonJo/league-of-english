...
## Next 3 (priority)
1. **단어 시험 파이프라인 완성.** PDF 단어장을 업로드하면 30문항·5지선다 세트를 생성·저장하고, 학생 시험/점수/복습 통계를 남깁니다.
2. **분석본 품질 대시보드.** 신고/추천 로그를 교사가 정리하고 “검수 완료/보완 필요” 태그를 붙일 수 있는 관리자 뷰와 알림 루프를 추가합니다.
3. **베타 배포 리허설.** Render/Vercel 환경 변수, `npm install --prefix client`를 포함한 빌드 명령, `/api/health` 체크, 이메일 자격 증명을 검증해 1주 내 공개 베타를 준비합니다.
