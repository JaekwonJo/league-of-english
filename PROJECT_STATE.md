...
## 2025-11-17 업데이트
- 문제학습 > 빈칸 생성이 원문과 다른 텍스트를 쓰던 2-23-11 이슈를 서버에서 재현했고, 원문이 비어 있으면 즉시 오류를 보여 더 이상 임의 지문을 만들지 않도록 막았어요.
- normalize 단계에서 선택한 지문 본문과 생성 결과를 1:1로 대조해 정답·보기·targetExpression이 원문 영문구와 완전히 일치할 때만 통과하도록 강화했어요.
- 길이가 150자 미만인 조각은 빈칸 후보에서 제외해 "원문보다 짧다"는 사용자 피드백을 줄이고, 필터에 걸린 지문은 로그에 자동 기록되도록 했어요.

### Current Stage
- 빈칸 생성기 QA(원문 검증·passage 필터) + 문서 파서 안정화, 모의고사 업로드 루틴은 종전 상태 유지.

### Next 3
1) 2-23-11 문서를 다시 파싱해 passage 메타데이터를 정상화하고 QA 로그 남기기 (진행 중)
2) 빈칸/요약/어법 생성 회귀 테스트 스위트에 "원문 대조" 케이스 추가 (대기)
3) 학습 화면에 "원문 없음" 친절 가이드와 재업로드 버튼 추가 (기획 중)

## Resolved (오늘)
- 빈칸 생성기 원문 검증: passage가 없으면 즉시 실패시키고 정답/보기/targetExpression이 원문 구절과 다르면 재시도하도록 서버 로직을 보강했습니다.
