...
## Next 3 (오늘 우선순위)
1. [P0] 랭킹 실데이터 시드 – 폴백 순위 대신 실제 LP/티어 분포를 자동 누적하도록 야간 배치를 마련합니다.
2. [P1] 라벨 변경 이력 뷰어 – 새 로그를 관리자 UI에서 검색·롤백할 수 있는 화면과 권한 필터를 추가합니다.
3. [P1] Playwright 데이터 시나리오 확장 – CI에서 항상 통과하도록 샘플 문서/단어장을 자동 시드하고 캡처 아카이브를 정리합니다.
...
- 독수리 마스코트에 주기적 눈 깜빡임/날개 플랩/탭 점프 상호작용을 추가해 모바일에서 상징 요소의 생동감을 높였습니다.
