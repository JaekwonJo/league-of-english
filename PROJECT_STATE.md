...
## Today’s Top 3 (2025-10-17)
1. **요약 생성 회복력 강화.** `problemSetService` 경로에서 요약 실패 로그를 수집하고, 새 지시문 힌트를 통과하지 못한 케이스를 재현하는 통합 테스트/리포트 대시보드를 만들어 재시도 이전에 원인을 볼 수 있게 하세요. (출제 안정성)
2. **신고 인사이트 보드 확장.** 유형·문서·사용자별 필터와 CSV 내보내기, 일괄 처리 버튼을 추가해 관리자들이 신고 추세를 빠르게 파악하도록 만들어요. (품질 의사결정)
3. **테마 접근성 자동화.** CRA 테마 토큰(라이트/다크)을 대상으로 대비(contrast) 테스트와 vis regression을 돌리는 스크립트를 추가해, 남은 정적 CSS도 자동으로 점검할 수 있게 하세요. (사용자 경험)
