...
## Next 3 (오늘 우선순위)
1. [P0] 모의고사/어휘 Playwright 시나리오 추가 – 파서/히어로 리뉴얼 이후 회귀를 자동화해야 베타 중 500을 사전에 잡을 수 있습니다.
2. [P1] 지문 이름 편집 UI 모달화 – 현재 `prompt` 기반이라 모바일·IME 입력이 깨지므로 inline 입력/검증/취소 흐름이 필요합니다.
3. [P1] 랭킹 API 시드·폴백 확장 – 샘플 데이터가 모자라면 500이 나는 현존 이슈를 서버 기본 시드와 빈 상태 응답으로 막아야 합니다.
...
- 지문 이름 편집이 `prompt` 기반이라 모바일/IME 입력 시 UX가 깨지고 취소/검증을 할 수 없습니다.
