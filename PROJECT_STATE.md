...
2. **학습 세트 부분 성공 처리.** 한 유형이 실패해도 통과한 문제는 즉시 제공하고, 실패 유형은 재시도 큐로 돌릴 수 있게 API와 UI 흐름을 바꿉니다.
3. **품질 피드백 + 대비 개선.** 학습 화면에 "도움이 됐어요"·"신고" 버튼과 관리자 큐를 붙이고, 라이트/다크 테마 대비도 함께 재정비합니다.

## Priorities (2025-10-13)
1. 빈칸/어휘 생성기가 검증 단계에서 멈추지 않도록 프롬프트·검증 로직을 완화해 오늘 학습 세트가 다시 돌아가게 만들어요.
2. 학습 세트 요청이 일부 유형 실패로 중단돼도 통과한 문제는 바로 돌려줄 수 있도록 부분 성공/재시도 흐름을 추가해요.
3. 문제 화면에 "도움이 됐어요"·"신고" 버튼과 관리자 큐, 라이트/다크 대비 조정을 묶어 품질 피드백 루프를 열어요.
