...
## Next 3 (priority)
1. **어법·어휘 보기 셔플 도입.** 항상 ③번이 정답이라 패턴 학습이 가능한 상태라서, 옵션 셔플 + 정답 재매핑을 추가해 공정성을 확보합니다.
2. **학습 실패 안내 강화.** 세트 생성이 부분적으로 실패했을 때 유형별 원인/진행률을 보여주는 토스트/패널을 만들어 학생이 어떤 문제를 못 받았는지 바로 알 수 있게 합니다.
3. **Render 헬스체크/SMTP 안정화.** `/api/health` 라우트와 메일 스모크 스크립트를 추가해 배포 타임아웃을 없애고, 환경 변수 변경 시 즉시 검증할 수 있게 합니다.

## Known issues
- Grammar·vocabulary 생성기는 아직 보기 순서를 섞지 않아 항상 ③번이 정답으로 고정돼요. 셔플 + 정답 재매핑을 붙여야 학생이 패턴을 악용하지 않습니다.
