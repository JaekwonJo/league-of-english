...
## Next 3 (2025-10-20)
1. **어법 문제 생성 메커니즘 전면 재구성.** 밑줄 다섯 개 추출, 오류 유형 분류, 해설/오답 근거를 한 파이프라인에서 다시 정의해요. 짧은 지문이 반복 보기를 만들거나 잘못된 밑줄을 고르는 현상을 없애고, OpenAI·fallback이 같은 규칙을 따르도록 설계합니다. (콘텐츠 신뢰도)
2. **문서 기반 fallback + OpenAI 경로 회귀 테스트 자동화.** 업로드 지문 → problemSetService → 학습 화면까지 통합 시뮬레이션을 추가해요. fallback과 gpt 경로가 같은 검증기를 공유하도록 만들어 어법/어휘 회귀를 조기에 잡습니다. (배포 안전성)
3. **WordNet gloss 한국어 사전 구축.** 지금은 영어 gloss가 그대로 남아 해설이 어색해요. 핵심 어휘별 한국어 뜻·반의어·예문을 정리해 fallback 해설도 자연스러운 한국어로 제공합니다. (학생 이해도)
4. **Wolgo 기출 기반 라벨 시드 검수.** `scripts/generate-grammar-labels.js`가 만들어 준 60여 개 문장을 QA해서 신뢰도 높은 학습 샘플로 확정하고, 추가 지문 라벨링 지침을 공유합니다. (데이터 자산)

## Known issues
- 어법 문제 생성 파이프라인이 밑줄 선택·오류 태깅·해설 포맷에서 일관성이 부족합니다. 특히 문장이 4개 이하인 지문에서는 같은 문장이 보기로 여러 번 등장하고, 밑줄 위치가 실제 오류와 어긋나는 경우가 있습니다.
