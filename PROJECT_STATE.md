...
## 2025-11-13 업데이트
- 분석 화면 중복 제거: 영어 제목 옆 한국어 요약 표시를 제거해 중복 문구 3회 노출 문제 해결. 문장별 상세 섹션은 요청에 따라 다시 표시했습니다.
- 빈칸 문제 원문 보존 강화: 서버 검증을 "빈칸만 제외 동일"로 엄격화(길이/문장수 관용치 제거).
- 모델 교체 용이화: `LOE_OPENAI_PRIMARY_MODEL/SECONDARY_MODEL/ANALYSIS_MODEL` 환경변수로 선택 가능.
- 카카오 가입 후 학교/학년 수정: `/api/users/profile` GET/PUT 추가, 프로필 페이지에 편집 UI 추가.
