...
### Current Stage
- **완성 단계**: 기출문제 업로드/파싱 안정화 + AI 문제 생성 품질 검증 완료. 이제 콘텐츠 확장과 **인터랙티브 튜터(문법/독해) 안정화**가 핵심 과제입니다.
...
- **배포**: Vercel 빌드 명령을 `npm run vercel-build`로 통일하고, 프런트 의존성(`react-router-dom`) 누락으로 발생하던 프로덕션 빌드 오류를 해결.
- **AI 독해 튜터 Fix (2025-12-01)**: `/documents` 목록 응답 형식(배열 vs {data})과 상세 조회 응답 형식({success,data})을 프런트에서 일관되게 처리해, **독해 튜터 지문 선택 화면에 업로드한 문서가 보이지 않던 문제**를 해결했습니다.
- **AI 독해 튜터 UX 개선 (2025-12-01)**: 독해 튜터 진입 플로우를 `문서 선택 → 지문(문제) 선택 → AI 대화형 해설` 3단계로 재구성했습니다. `/analysis/status/:documentId`와 `/analysis/:documentId/passage-list` API를 활용해, 문서별 지문 개수와 각 지문의 미리보기(발췌, 단어 수)를 보여준 뒤, 선택한 지문만 문장 단위로 쪼개서 AI 튜터가 한 줄씩 안내하도록 바꿨습니다.
- **문서 업로드/정렬 개선 (2025-12-01)**: 관리자 문서 업로드 모달에 **여러 PDF/TXT를 한 번에 선택해 순차 업로드하는 멀티 업로드 기능**과, 문서 목록을 **최신순/가나다순/연도별**로 정렬할 수 있는 옵션을 추가했습니다. 동시에 독해 튜터 문서/지문 선택 화면과 채팅창에 Aurora 다크 + Glass 카드 스타일과 부드러운 hover/배경 애니메이션을 적용해, 공부 화면의 몰입감을 높였습니다.
- **AI 워크북 튜터 통합 (2025-12-01)**: 기존 카드형 워크북 페이지를 정리하고, 독해 튜터에서 지문을 선택한 뒤 각 지문 카드 아래의 `🤖 AI 워크북` 버튼으로 10단계 워크북을 바로 시작하도록 플로우를 단일화했습니다. `/study/ai-workbook/chat`, `AIWorkbookPage`, `WorkbookPage`를 하나의 흐름으로 묶어, 학생 입장에서는 \"지문 선택 → AI 독해 → AI 워크북\"으로 자연스럽게 이어지도록 했습니다.
