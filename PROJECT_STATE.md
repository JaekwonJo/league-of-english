...
## Today’s Top 3 (2025-10-13)
1. **빈칸·어휘 파이프라인 안정화.** 빈칸·어휘 검증이 조금 어긋나도 재시도·보정 루프로 넘겨 "문제를 준비하지 못했어요" 중단을 막아요. (핵심 기능 복구)
2. **학습 세트 부분 성공 처리.** 학습 세트가 부분 성공해도 통과한 문제는 즉시 제공하고 실패 유형만 재시도 큐로 돌려요. (학생 경험 안정)
3. **품질 피드백 + 대비 개선.** 문제 화면에 "도움이 됐어요"·"신고" 버튼과 관리자 큐를 붙이고, 라이트/다크 대비를 다시 정리해요. (품질 피드백 + 가독성)
