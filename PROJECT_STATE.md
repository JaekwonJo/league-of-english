...
- 분석/관리자 리스트 UI를 감성 카드·그라데이션 톤으로 통일해 모바일과 데스크톱 모두 같은 톤앤매너를 유지합니다.
...
- 관리자 포털 UI가 감성 카드 스타일로 정리돼 문서/모의고사/신고 보드를 일관된 UX로 운영할 수 있습니다.
- 자동 배포(Render/Vercel)는 토큰·시크릿 보강 이후 연결 예정입니다.

## Next 3 (오늘 우선순위)
1. 모의고사 실전 모드 업로드→응시→해설 흐름을 Playwright 시나리오로 자동화해 PDF 파서/타이머 회귀를 감시합니다.
2. Render/Vercel 시크릿(`RENDER_DEPLOY_HOOK_URL`, `VERCEL_*`)과 GitHub PAT를 정리해 main 푸시 시 자동 배포를 복구합니다.
3. 랭킹 API(leaderboard·tier-distribution) 데이터 시드를 보강하고 실패 시 대체 응답을 반환하도록 패칭합니다.

## Known Issues
- Playwright 스냅샷/시나리오가 없어 새 워크북·모의고사 플로우 회귀를 사람이 직접 확인해야 합니다.
...
- SMTP 자격 증명 미연결 상태라 이메일 인증/패스워드 초기화를 베타 우회 채널로 처리합니다.
- 모의고사 업로드는 현재 단일 회차(`2025-10`)만 지원해 여러 회차를 보관/선택하는 UI가 필요합니다.
- Workbook bulk 생성 결과 알림은 로그만 출력되어, 실패 건수를 UI에서 바로 확인하기 어렵습니다.

## Resolved – 2025-11-06 (오류 반복 방지)
- 분석 자료·관리자 리스트를 감성 카드/그라데이션 UI로 통일해 테마 혼선을 제거했습니다.
