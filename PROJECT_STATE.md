...
## Next 3 (오늘 우선순위)
1. [P0] Playwright 자동화 CI 연결 – `npm run dev:auto` + `npm run test:e2e` 조합을 GitHub Actions에 붙여 릴리스 전 E2E 스냅샷을 자동으로 남깁니다.
2. [P1] 지문 이름 변경 이력/권한 감사 – 모달 입력은 완성됐으므로 변경 로그와 관리자 권한 체크를 DB에 남겨야 합니다.
3. [P1] 랭킹 실데이터 시드 + LP 싱크 – 폴백은 준비됐지만 실제 사용자 LP/티어를 채우는 배치와 시드를 자동화해야 합니다.

## Known Issues
- Playwright E2E는 로컬 `npm run dev:auto` 환경에서만 실행되며 아직 CI/배포 파이프라인에 연결되지 않았습니다.
- 랭킹 API는 데이터가 없을 때 프리셋 값을 돌려주므로, 실제 학습 데이터가 적재되기 전까지는 데모 정보가 노출됩니다.
- 지문 이름 변경은 이제 모달로 이뤄지지만 변경 이력과 권한 감사 로그가 없어 추적이 어렵습니다.
