...
2. **Progress & Tasks Strip**
   ...
   - Each block gets a tiny OwlGuide chip (“🦉 복습부터!”) to reinforce the mascot persona.
...
3. **Guided Rituals (Review Queue, Vocabulary, Mock Exam)**
   ...
   - Upcoming work: embed micro-confetti when 복습 완료, and show owl “sleep” animation when queue is empty.

**Implementation Notes**
- New `owlHints` array + timed rotation already live; extend with per-user context once API exposes streak data.
